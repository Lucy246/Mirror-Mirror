...
var Weather = React.createClass({
    ...
    fetchData() {
        ...
        var forecastEndpoint = 'forecast/daily';
        var apiVersion = '2.5';
        var self = this;

        ajax({
            type: 'GET',
            url: apiBase + '/' + apiVersion + '/' + weatherEndpoint,
            dataType: 'json',
            data: {
                q: this.props.q,
            ...
            success: function (data) {
                var _temperature = self.roundValue(data.main.temp),
                    _temperatureMin = self.roundValue(data.main.temp_min),
                    _temperatureMax = self.roundValue(data.main.temp_max),
                    _wind = self.roundValue(data.wind.speed),
                    _iconClass = self._iconTable[data.weather[0].icon];
    ...
    render() {
        var newSunHtml = (<div><span className="wi wi-sunrise xdimmed"></span> <span>{this.state.sunrise}</span></div>);

        if (this.state.sunrise < this.state.now && this.state.sunset > this.state.now) {
            newSunHtml = (<div><span className="wi wi-sunset xdimmed"></span> <span>{this.state.sunset}</span></div>);
        }

        return (
            <section className='Card WeatherCard'>
                <div className="windsun small dimmed">
                    <span className="wi wi-strong-wind xdimmed"></span> <span>{this.ms2Beaufort(this.state.wind)}</span>
                    <span>{newSunHtml}</span>
                </div>
                <div className="temp">
                    <span className={"icon " + this.state.iconClass + " dimmed wi"}></span>
                    <span>{this.state.temperature + '&deg;'} </span>
                </div>
                <div className="forecast small dimmed">
